...
- **🔐 Secure by Default**: Full SSL/TLS encryption with auto-generated certificates
- **⚡ One-Command Setup**: Fully automated installation and configuration
- **🔄 Fleet Server Ready**: Pre-configured Fleet Server for centralized agent management
- **📊 Complete Monitoring**: Elasticsearch, Kibana, and Fleet Server with health checks
- **🛠️ Management Script**: Comprehensive CLI tool for lifecycle management
- **📦 Docker Compose Based**: Easy deployment with profile-based service management
- **🔑 Auto Token Management**: Automatic Fleet service token generation and configuration
- **📝 Persistent Logging**: Centralized logging for all services
...
- ✅ Configure Kibana with encryption keys
- ✅ Set up Fleet Server with service token
...
Configure Fleet Server and Elasticsearch addresses for agent enrollment:

| Variable | Description | Default |
|----------|-------------|---------|
| `FLEET_SERVER_HOST` | Primary Fleet Server URL | `https://fleet-server:8220` |
| `FLEET_SERVER_EXTERNAL_HOST` | External Fleet Server URL for remote agents | (not set) |
| `ELASTICSEARCH_OUTPUT_HOST` | Elasticsearch output for agents | `https://elasticsearch:9200` |

**Examples**:

```bash
# Use HOST_IP for external access
FLEET_SERVER_HOST=https://192.168.1.100:8220
ELASTICSEARCH_OUTPUT_HOST=https://192.168.1.100:9200

# Or add a second external host for remote agents
...
├── elastic-stack.sh           # Management script
...
|---------|------|-----------------|
| Elasticsearch | `elastic` | `.env` → `ELASTIC_PASSWORD` |
| Kibana System | `kibana_system` | `.env` → `KIBANA_PASSWORD` |
...
# Fleet → Add agent → Copy enrollment command
